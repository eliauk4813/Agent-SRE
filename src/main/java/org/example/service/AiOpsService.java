...
public class AiOpsService {
    ...
    @Autowired(required = false)  // 本地日志工具可用时注册；真实环境也可直接走本工具查询 CLS
    ...
    private String buildPlannerPrompt() {
        return """
                ...
                2. 先调用 queryPrometheusAlerts 获取活跃告警，再根据告警名称、服务名、时间窗优先调用 queryLogsByAlert 获取日志证据；只有在需要追加精确检索时才调用 queryLogs 或 queryLogsByTraceId。
                3. 在拿到日志证据后，再调用 queryInternalDocs 检索对应告警或故障处理文档，形成“告警 -> 日志 -> 文档”的证据链。
                4. 在执行阶段，输出 JSON，包含 decision (PLAN|EXECUTE|FINISH)、step 描述、预期要调用的工具、以及必要的上下文。
                5. 调用任何腾讯云日志工具时，region 参数必须使用连字符格式（如 ap-guangzhou），若不确定请省略以使用默认值。
    ...
    private String buildExecutorPrompt() {
        return """
                ...
                - 如果步骤是日志排查，优先调用 queryLogsByAlert；如果已经拿到 traceId，则调用 queryLogsByTraceId；仅在需要补充精确条件时调用 queryLogs。
                - 调用相应的工具并收集结果，如工具返回错误或空数据，需要将失败原因、请求参数一并记录，并停止进一步调用该工具（同一工具失败达到 3 次时应直接返回 FAILED）。
                - 将日志、指标、文档等证据整理成结构化摘要，优先引用工具返回的 summary、statistics、evidenceSamples，标注对应的告警名称或资源，方便 Planner 填充"告警根因分析 / 处理方案执行"章节。
    ...
    private String buildSupervisorSystemPrompt() {
        return """
                ...
                3. 标准执行顺序是：先查 queryPrometheusAlerts，再按告警调用 queryLogsByAlert 获取日志证据，再调用 queryInternalDocs 查询处理文档；必要时追加 queryLogs 或 queryLogsByTraceId。
                4. 根据 executor_agent 的反馈，评估是否需要再次调用 planner_agent，直到 decision=FINISH。
                5. FINISH 后，确保向最终用户输出完整的《告警分析报告》，格式必须严格为：
                   告警分析报告\n---\n# 告警处理详情\n## 活跃告警清单\n## 告警根因分析N\n## 处理方案执行N\n## 结论。
                6. 若步骤涉及腾讯云日志工具，请确保使用连字符区域 ID（ap-guangzhou 等），或省略 region 以采用默认值。
